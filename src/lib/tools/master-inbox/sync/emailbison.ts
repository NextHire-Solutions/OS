...
import { createEmailBisonClient, type ConvReply } from "@/lib/tools/master-inbox/emailbison/client";
...
import type {
  ...
  EmailBisonLead,
} from "@/lib/tools/master-inbox/emailbison/types";
import {
  inboundFromReply,
  leadFields,
...
// Ported from the tool's lib/sync/emailbison.ts. The envelope → row mapping
// lives in ./emailbison-rows.ts so it can be tested without a database; the
...
// RELEVANT_EVENTS in emailbison/types.ts). Everything else is ignored.
//
...
async function backfillConversation(
  ...
  try {
    ...
    for (const s of sent) {
      ...
      await upsertMessage({
        ...
        threadId,
        direction: "outbound",
        externalMessageId: scheduledExternalId(s.id),
        sender: senderAddr,
  ...
  try {
    ...
    for (const r of all) {
      ...
      // /threads/[threadId]/reply writes, so backfill becomes an
      // idempotent update instead of producing a duplicate row.
      const externalMessageId = replyExternalId(r.id);
...
export async function handleEmailBisonEvent(envelope: EmailBisonWebhookEnvelope): Promise<{
  ...
  const { eventBlock, payload, eventType } = unwrapEnvelope(envelope);
