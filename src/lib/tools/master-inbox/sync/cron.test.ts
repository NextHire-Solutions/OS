...
import { JOB_NAMES, SCHEDULE, SYNC_EXTERNAL_INTROS, cronEnabled, dueMasterInboxJobs, minuteKey } from "./cron.ts";

const at = (hh: number, mm: number, ss = 0) => new Date(Date.UTC(2026, 8, 15, hh, mm, ss));

test("the one job the tool scheduled is the one job scheduled here", () => {
  assert.deepEqual(JOB_NAMES, [SYNC_EXTERNAL_INTROS]);
...
test("due on the half hour, not in between", () => {
  assert.deepEqual(dueMasterInboxJobs(at(14, 0)), [SYNC_EXTERNAL_INTROS]);
  assert.deepEqual(dueMasterInboxJobs(at(14, 30)), [SYNC_EXTERNAL_INTROS]);
...
test("a late tick still sees the job within its window", () => {
  ...
  assert.deepEqual(dueMasterInboxJobs(at(14, 30, 40)), [SYNC_EXTERNAL_INTROS]);
  // And a tick a minute late catches it only if told to look back.
  assert.deepEqual(dueMasterInboxJobs(at(14, 31), 1), []);
  assert.deepEqual(dueMasterInboxJobs(at(14, 31), 2), [SYNC_EXTERNAL_INTROS]);
});

test("one-minute windows tile the day: 48 runs, none doubled", () => {
  let runs = 0;
  for (let minute = 0; minute < 24 * 60; minute++) {
    runs += dueMasterInboxJobs(new Date(Date.UTC(2026, 8, 15, 0, 0) + minute * 60_000), 1).length;
  }
