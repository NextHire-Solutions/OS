...
export function createEmailBisonClient(): EmailBisonClient {
  const baseUrl = (
    process.env.ANALYTICS_EMAILBISON_BASE_URL || "https://send.brokerstaffer.com"
  ).replace(/\/$/, "");
  const apiKey = process.env.ANALYTICS_EMAILBISON_API_KEY;

  if (!apiKey) {
    throw new Error(
      "ANALYTICS_EMAILBISON_API_KEY is not set. See .env.example.",
    );
  }
