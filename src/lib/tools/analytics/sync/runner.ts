 ...
 *
 * One deliberate difference from the tool: the Supabase client is a trailing
 * parameter that defaults to the real one. Every caller in the app omits it,
 * so behaviour is identical; the tests pass a recording stub, which is the
 * only way to exercise the lock, the stale-lock theft and the circuit breaker
 * without a database. The tool has no tests for this file for exactly that
 * reason.
 */
