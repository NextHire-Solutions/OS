...
//
// PORTED from the tool's lib/instantly.ts (eb7d572) with one change: the API
// key is read from CLIENT_HEALTH_INSTANTLY_API_KEY, because five tools share
// this process and Master Inbox already owns the unprefixed name.
...
function key(): string {
  const k = optionalEnv('CLIENT_HEALTH_INSTANTLY_API_KEY');
  if (!k) throw new Error('CLIENT_HEALTH_INSTANTLY_API_KEY is not set');
  return k;
}
