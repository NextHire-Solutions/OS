...
/*
 * The rich-text editor is loaded on demand.
 *
 * ---------------------------------------------------------------------------
 * WHY
 *
 * TipTap and ProseMirror are the single largest thing in the client bundle.
 * Statically imported, they were downloaded and parsed by everyone who opened
 * ANY screen in the workspace — the OS shell renders every screen into one
 * document, so an import in the composer is an import on the home page.
 *
 * Almost nobody needs it on arrival: it is required the moment someone presses
 * Reply, and not before. `next/dynamic` moves it to its own chunk fetched at
 * that point, which is fast on a warm connection and invisible next to the
 * time it takes to start typing.
 *
 * `ssr: false` because the editor mounts against a DOM node and has nothing to
 * contribute to server HTML; rendering it on the server only produces markup
 * that is immediately replaced.
 *
 * The `loading` state matches the editor's own height so the composer does not
 * jump when the chunk lands.
 */
const ComposerBodyEditor = dynamic(
  () =>
    import("@/components/master-inbox/composer-body-editor").then(
      (m) => m.ComposerBodyEditor,
    ),
  {
    ssr: false,
    loading: () => (
      <div className="min-h-[160px] rounded-md border bg-background" aria-busy="true" />
    ),
  },
);
...
export function Composer({
  ...
  async function saveDraft(payload: DraftPayload, key: string) {
    ...
    try {
      const res = await fetch(`/api/tools/master-inbox/threads/${threadId}/composer-draft`, {
        method: "PUT",
      ...
      try {
        ...
        if (typeof navigator !== "undefined" && navigator.sendBeacon) {
          navigator.sendBeacon(
            `/api/tools/master-inbox/threads/${threadId}/composer-draft`,
            body,
          );
        } else {
          void fetch(`/api/tools/master-inbox/threads/${threadId}/composer-draft`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(payload),
            keepalive: true,
  ...
  async function applyIntroductionLabelIfSent(sentBody: string): Promise<boolean> {
    ...
    try {
      const res = await fetch(`/api/tools/master-inbox/threads/${threadId}/labels`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function generateAiReply() {
    ...
    setGenerating(true);
    try {
      const res = await fetch(`/api/tools/master-inbox/threads/${threadId}/generate-draft`, {
        method: "POST",
      });
  ...
  async function onSend() {
    ...
    try {
      ...
      if (files.length > 0) {
        ...
        res = await fetch(`/api/tools/master-inbox/threads/${threadId}/reply`, {
          method: "POST",
          body: form,
        });
      } else {
...
function formatQuotedTimestamp(ts: string | null): string {
  ...
  return d.toLocaleString("en-US", {
    // Explicit locale + zone: see thread-view.tsx formatTime.
...
function TemplatePicker({
  ...
  async function ensureLoaded() {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/tools/master-inbox/reply-templates", { cache: "no-store" });
