...
/*
 * EmailBison's sending schedules, once a minute per day-key rather than on
 * every request. The call was live on each load and was measured at 5.1s on
 * one production run (0.4s on the next) — the screen's whole delay. A
 * minute-old schedule is the same schedule; the stale window means a slow
 * EmailBison answer is paid in the background, not by the person waiting.
 */
const cachedSchedules = ttlCache(
  (day: string) => createEmailBisonClient().getAllSendingSchedules(day as "today" | "tomorrow" | "day_after_tomorrow"),
  { ttlMs: 60_000, staleMs: 10 * 60_000, key: (day) => day },
);
...
export async function GET(request: NextRequest) {
  ...
  try {
    schedules = await cachedSchedules(day);
  } catch (error) {
