...
import { _invalidateClientCache, deriveClientIdFromCampaign } from "@/lib/tools/master-inbox/clients/derive";
...
export async function requireAuthedUser() {
  /*
   * ---------------------------------------------------------------------
   * PORTED, NOT COPIED — this one could not stay as it was.
   *
   * The tool asks Supabase who the caller is. The OS has no Supabase user —
   * reads go through a service-role client that is nobody — so that call
   * returns null every time and this helper returned 401 on every request.
   * `/clients`, `/clients/portals` and `/clients/intro-stats` were all dead
   * until this was fixed, which is the failure mode of copying code without
   * exercising it.
   *
   * The OS authenticates at its own front door: `proxy.ts` verifies the
   * signed `bs_sso` cookie before any route here is reachable. This
   * re-verifies rather than trusting that, because the proxy's matcher is one
   * regex away from excluding a path, and verification is a single HMAC.
   *
   * `id: null` is deliberate — see src/lib/auth/workspace.ts.
   * ---------------------------------------------------------------------
   */
  ...
  if (!hasGrant(session, "inbox")) {
    return {
      error: NextResponse.json(
        { error: "You do not have access to Master Inbox." },
        { status: 403 },
      ),
    };
  }
  return { user: { id: null, email: session.email } };
}
