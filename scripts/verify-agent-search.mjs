 ...
 *
 * This booted Agent Search as its OWN deployed app under `apps/agent-search` and proved it
 ...
 * everything should be done from our OS only." Agent Search is built into the OS and
...
if (!__exists(new URL("../apps/agent-search", import.meta.url).pathname)) {
  console.log("SKIP  verify-agent-search.mjs — apps/agent-search no longer exists.");
  console.log("      Agent Search is built into the OS; this tested the separate");
...
/*
 * Dynamic, because a static import is hoisted above the skip guard above and
 * would crash on the missing path before the guard could report it.
 */
const {
  hasGrant: jsHasGrant,
  readSsoCookie: jsReadCookie,
  verifySso: jsVerify,
} = await import("../apps/agent-search/web/server/bs-auth.js");
